import fs from 'fs';
// node src\data\products\zakonczenie_komina\zakonczenie_komina_config.mjs
...
const addVariants = (type, grade, thick, exec, ins, diameters) => {
    diameters.forEach(d => {
        ...
        const sku = `ZK-07-${typeCode}-${gradeCode}-${thickCode}-${execCode}${insPart}-${safeSkuDiam}`;
...
fs.writeFileSync('zakonczenie_komina.json', jsonString, 'utf-8');